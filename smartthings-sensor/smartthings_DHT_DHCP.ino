/*
    ...
    Added DHT support and extra debugging lines  JPB 12/22/2016
...
void sendTempJSONData(WiFiClient client) {
  ...
  //Use for testing without sensor
...
void sendHumidJSONData(WiFiClient client) {

  String humidString = String(getHumid());

  //Use for testing without sensor

  //String humidString = "00";
  ...
  client.println(28+humidString.length());

  client.println();

  client.print(F("{\"name\":\"humidity\",\"value\":"));

  client.print(humidString);
  ...
  while (client.connected()) {
    ...
    if (client.available()) {
      ...
      if (c == '\n' && currentLineIsBlank) {
        ...
        } else if (readString.substring(readString.indexOf('/'), readString.indexOf('/') + 9) == "/getHumid") {

          client.println("HTTP/1.1 200 OK"); //send new page

          sendHumidJSONData(client);
